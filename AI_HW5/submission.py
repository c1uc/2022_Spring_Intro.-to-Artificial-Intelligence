...
class ExactInference(object):
    ...
    def elapseTime(self) -> None:
        ...
        """new belief"""
        tmp = util.Belief(self.belief.getNumRows(), self.belief.getNumCols(), 0)
        for trans in self.transProb.items():
            """tiles before and after transition"""
            old, new = trans[0]

            """transition probability"""
            prob = trans[1]

            """update P(H_t+1|E) according to P(H_t|E)"""
            post = self.belief.getProb(old[0], old[1])
            tmp.addProb(new[0], new[1], post * prob)
...
class ParticleFilter(object):
    ...
    def updateBelief(self) -> None:
        ...
        for tile, prob in self.particles.items():
            newBelief.setProb(tile[0], tile[1], prob)
        newBelief.normalize()
    ...
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        ...
        """calculate P(H_t|E_t) according to E_t and P(E_t|H_t)"""
        """updated P(H_t|E_t) dict"""
        particlesProb = dict()
        for tile, count in self.particles.items():
            """if there is no particle"""
            if self.particles[tile] == 0:
                continue

            """calculate the distance and probability"""
            dist = math.sqrt((agentX - util.colToX(tile[1])) ** 2 + (agentY - util.rowToY(tile[0])) ** 2)
            prob = util.pdf(dist, Const.SONAR_STD, observedDist)

            """P(particle on the tile) = P(particle is on the tile|E_t) * #(sampled particles on the tile)"""
            particlesProb[tile] = prob * count

        newParticles = collections.defaultdict(int)
        for _ in range(self.NUM_PARTICLES):
            """resample the particles"""
            newParticles[util.weightedRandomChoice(particlesProb)] += 1
    ...
    def elapseTime(self) -> None:
        ...
        """for the particles on each tile"""
        for oldTile, count in self.particles.items():
            if oldTile not in self.transProbDict:
                continue

            """get the transition probability of each tile at time t+1 according to time t"""
            probs = self.transProbDict[oldTile]

            for _ in range(count):
                """randomly choose new tile for each particle on the tile at time t"""
                newParticles[util.weightedRandomChoice(probs)] += 1
        self.particles = newParticles
        # END_YOUR_CODE
