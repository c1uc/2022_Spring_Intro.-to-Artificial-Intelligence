...
class Agent():
    ...
    def discretize_observation(self, observation):
        ...
        """
        # Begin your code
        return [self.discretize_value(observation[ind], self.bins[ind]) for ind in range(len(observation))]
        # End your code
    ...
    def choose_action(self, state):
        ...
        r = rng.random()
        if r < self.epsilon:
            return rng.integers(0, self.env.action_space.n)
        else:
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        def learn_test():
            ...
            new_Agent.qtable = self.qtable

            t_state = new_Agent.discretize_observation(new_Agent.env.reset())
            count = 0
            while True:
                count += 1
                act = np.argmax(new_Agent.qtable[tuple(t_state)])
                n_o, _, d, _ = new_Agent.env.step(act)

                n_state = new_Agent.discretize_observation(n_o)
        ...
        if done:
            self.qtable[tuple(state)][action] = reward
        else:
            self.qtable[tuple(state)][action] = \
                (1 - self.learning_rate) * self.qtable[tuple(state)][action] + \
    ...
    def check_max_Q(self):
        ...
        # Begin your code
        st = self.env.reset()
        state = self.discretize_observation(st)
