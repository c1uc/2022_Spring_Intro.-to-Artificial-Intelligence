...
class Agent():
    ...
    def learn(self):
        ...
        def learn_test():
            ...
            while True:
                count += 1
                q = new_Agent.target_net.forward(
                    torch.FloatTensor(t_state)).squeeze(0).detach()
                act = int(torch.argmax(q).numpy())
                n_state, _, d, _ = t_env.step(act)
        ...
        states = torch.tensor(numpy.array(observations), dtype=torch.float)
        next_states = torch.tensor(numpy.array(next_observations), dtype=torch.float)
        re = torch.tensor(rewards, dtype=torch.float).view(self.batch_size, -1)
        acts = torch.tensor(actions, dtype=torch.long).view(self.batch_size, -1)

        not_done = [a[0] for a in enumerate(done) if a[1] == 0]
        not_done_states = torch.cat([next_states[a] for a in not_done]).view(-1, 4)
    ...
    def check_max_Q(self):
        ...
        return torch.max(self.evaluate_net(
            torch.FloatTensor(state).squeeze(0).detach()
        )).item()
        # End your code
