...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        """
        for each feature (weak classifier), compute its error and choose the best weak classifier.
        because error always <= 1.0, so best error is assigned 1.0 by default.
        the feature of the best weak classifier will be modified when best error changed.  
        """
        bestError = 1.0
        bestClf = WeakClassifier(feature=None)

        for i in range(len(features)):
            # the weak classifier to be tested, and compute its error
            clf = WeakClassifier(feature=features[i])
            err = sum([weights[j] * abs(clf.classify(iis[j]) - labels[j]) for j in range(len(labels))])
            if err < bestError:
                # if this error is lower than the best one, then modify the result.
                bestClf = clf
                bestError = err
